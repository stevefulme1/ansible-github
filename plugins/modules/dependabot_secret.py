...
DOCUMENTATION = r"""
---
module: dependabot_secret
short_description: Manage dependabot
version_added: "1.0.0"
description:
  - Create, update, and delete dependabot_secret resources.
...
options:
  state:
    description:
      - Desired state of the dependabot_secret resource.
    type: str
...
def get_current_state(client, module):
    """Retrieve the current state of the dependabot_secret via GET."""
        ...
        if state == "present":
            ...
            if current is not None:
                ...
                if not module.check_mode:
                    ...
                    identifier = current.get("id")
                    path = "/repos/{owner}/{repo}/dependabot/secrets/{secret_name}".replace(
                        "{id}", str(identifier)
                    )
                    client.delete(path)
