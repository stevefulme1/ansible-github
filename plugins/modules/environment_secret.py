...
DOCUMENTATION = r"""
---
module: environment_secret
short_description: Manage actions
version_added: "1.0.0"
description:
  - Create, update, and delete environment_secret resources.
...
options:
  state:
    description:
      - Desired state of the environment_secret resource.
    type: str
  ...
  encrypted_value:
    description:
      - >-
        Value for your secret, encrypted with LibSodium using the public key retrieved from the Get an...
    type: str
...
def get_current_state(client, module):
    """Retrieve the current state of the environment_secret via GET."""
        ...
        if state == "present":
            ...
            if current is not None:
                ...
                if not module.check_mode:
                    ...
                    identifier = current.get("id")
                    path = "/repos/{owner}/{repo}/environments/{environment_name}/secrets/{secret_name}".replace(
                        "{id}", str(identifier)
                    )
                    client.delete(path)
