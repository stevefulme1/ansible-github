"""Unit tests for stevefulme1.github.action_runner_group module."""
...
MODULE_PATH = "ansible_collections.stevefulme1.github.plugins.modules.action_runner_group"
CLIENT_PATH = "ansible_collections.stevefulme1.github.plugins.module_utils.api_client"


def _build_resource(**overrides):
    """Return a mock action_runner_group resource dict."""
    base = {
        "id": "res-123",
        "name": "test-name",
        "allows_public_repositories": "test-allows_public_repositories",
        "network_configuration_id": "test-network_configuration_id",
        "restricted_to_workflows": "test-restricted_to_workflows"
    }
...
def resource_args(module_args):
    """Module args for action_runner_group operations."""
    module_args.update({
        "state": "present",
...
class TestGetCurrentState:
    ...
    def test_returns_matching_resource(self, resource_args):
        ...
        from ansible_collections.stevefulme1.github.plugins.modules.action_runner_group import get_current_state
    ...
    def test_handles_client_error(self, resource_args):
        ...
        resource_args["id"] = None
        from ansible_collections.stevefulme1.github.plugins.modules.action_runner_group import get_current_state
        from ansible_collections.stevefulme1.github.plugins.module_utils.api_client import ClientError
...
class TestNeedsUpdate:
    ...
    def test_returns_true_when_no_current(self):
        ...
        from ansible_collections.stevefulme1.github.plugins.modules.action_runner_group import needs_update
...
class TestBuildPayload:
    ...
    def test_builds_payload_from_params(self, resource_args):
        ...
        from ansible_collections.stevefulme1.github.plugins.modules.action_runner_group import build_payload
...
class TestCreate:
    ...
    def test_create_sets_changed(self, mock_ansible_cls, mock_client_cls, resource_args):
        ...
        with patch(f"{MODULE_PATH}.get_current_state", return_value=None):
            from ansible_collections.stevefulme1.github.plugins.modules.action_runner_group import main
            main()
...
class TestUpdate:
    ...
    def test_update_when_changed(self, mock_ansible_cls, mock_client_cls, resource_args):
        ...
        resource_args["allows_public_repositories"] = "new-value"
        mock_module = MagicMock()
        mock_module.params = resource_args
        mock_module.check_mode = False
        mock_ansible_cls.return_value = mock_module
        ...
        mock_client = MagicMock()
        mock_client.put.return_value = _build_resource(allows_public_repositories="new-value")
        mock_client_cls.return_value = mock_client

        existing = _build_resource(allows_public_repositories="old-value")
        with patch(f"{MODULE_PATH}.get_current_state", return_value=existing), \
